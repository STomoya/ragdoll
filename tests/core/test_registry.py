...
def test_register_rejects_bare_dict_config():
    registry = StageRegistry()
    with pytest.raises(InvalidConfigModelError):
...
def test_declare_incompatible():
    declare_incompatible('graph_chunker', 'bm25', 'graph chunks require a graph-aware retriever')
    assert is_compatible('graph_chunker', 'bm25') is False
    assert incompatibility_reason('graph_chunker', 'bm25') == 'graph chunks require a graph-aware retriever'
